...
class EPTAttentionMoT(nn.Module):
    ...
    def forward(
        ...
        if text_k is None or text_v is None:
            L_text_max = 0
            text_k = H.new_zeros(B, 0, self.n_kv_heads, self.d_qk_head)
            # text_v must match V_attn_vae dimension: 4*d_head (scalar + 3*vector)
            text_v = H.new_zeros(B, 0, self.n_kv_heads, 4 * self.d_head)
